...
const INSTAGRAM_JSONS_FOLDER = "./jsons";
...
export class InstagramService {
  ...
  async postData(
    ...
    if (createMode && results.filter(Boolean).length > 0) {
      results
        .filter(Boolean)
        .forEach((x) => (x.description = x.description || ""));
      const response = await this.tinderService.createBulk(
        results,
        downloadMedia
      );
      ...
      createdData.totals.downloadedImages =
        createdData.totals.downloadedImages || 0;
      createdData.totals.downloadedVideos =
        createdData.totals.downloadedVideos || 0;
      createdData.totals.downloadedImages +=
        response?.data?.downloaded?.images ?? 0;
      createdData.totals.downloadedVideos +=
        response?.data?.downloaded?.videos ?? 0;
  ...
  async getSavedPostsData(createMode: boolean = false) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (currPromises.length === maxInParallel) {
        const results =
          (await Promise.all(currPromises).catch((error) => {
            allFailed.push({
              url: error?.config?.url,
              error: error?.message,
            });
  ...
  async createInstagramDataFromJSON(
    itemsJSON: any,
    downloadMedia: boolean = true
  ) {
    const results = itemsJSON?.items?.map((item) =>
      this.scrapeInstagramJSON(item)
    );
    let createdData = {
      totals: {
        created: 0,
    ...
    createdData = await this.postData(
      true,
      results,
      createdData,
      downloadMedia
  ...
  async scrapeInstagramProfile(
    userId: string
  ): Promise<CreateInstagramItemsResult> {
    // travel bloggers
    ...
    return await this.recursivelyScrapePage(userId, baseUrl, 1);
  }

  async recursivelyScrapePage(
    userId: string,
    baseUrl: string,
    reqCount: number,
    max_id?: string
    ...
    // backup json
    if (!fs.existsSync(INSTAGRAM_JSONS_FOLDER)) {
      fs.mkdirSync(INSTAGRAM_JSONS_FOLDER);
    }
    if (!fs.existsSync(`${INSTAGRAM_JSONS_FOLDER}/${userId}`)) {
      fs.mkdirSync(`${INSTAGRAM_JSONS_FOLDER}/${userId}`);
    }
    const filePath = max_id
      ? `${INSTAGRAM_JSONS_FOLDER}/${userId}/req_${reqCount}_maxId_${max_id}.json`
      : `${INSTAGRAM_JSONS_FOLDER}/${userId}/req_${reqCount}.json`;
    if (!fs.existsSync(filePath)) {
      await this.createFile(filePath, JSON.stringify(json));
    }
    ...
    if (json["more_available"] && json["next_max_id"]) {
      const subResult: CreateInstagramItemsResult = await this.recursivelyScrapePage(
        userId,
        baseUrl,
        reqCount + 1,
        json["next_max_id"]
