...
exports.SpaceJamService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const class_validator_1 = require("class-validator");
const space_jam_repository_1 = require("./space-jam.repository");
const space_jam_player_service_1 = require("../../space-jam-player/space-jam-player.service");
let SpaceJamService = class SpaceJamService {
    constructor(spaceJamRepository, playerService) {
        this.spaceJamRepository = spaceJamRepository;
        this.playerService = playerService;
        this.logger = new common_1.Logger('SpaceJamService');
    }
    async createRecord(createDto, user) {
        ...
        const player1 = await this.playerService.searchPlayer(name1);
        const player2 = await this.playerService.searchPlayer(name2);
        const record = await this.spaceJamRepository.createRecord(createDto, player1, player2, user);
        const listDto = {};
        const stats = await this.listRecordsByPlayer(listDto, user);
        const nickname1 = player1.nickname;
        const nickname2 = player2.nickname;
        const toReturn = {};
        toReturn[nickname1] = stats[nickname1];
        toReturn[nickname2] = stats[nickname2];
        return toReturn;
    }
    async deleteRecord(id, user) {
        const result = await this.spaceJamRepository.delete({
            id: id,
            userId: user.id,
        });
    ...
    async updateRecord(id, updateDto, user) {
        const record = await this.spaceJamRepository.findOne({
            id: id,
            userId: user.id,
        });
        if (!record) {
        ...
        const player1 = await this.playerService.searchPlayer(record.player1);
        const player2 = await this.playerService.searchPlayer(record.player2);
        if (!class_validator_1.isDefined(updateDto.score1) && !class_validator_1.isDefined(updateDto.score2)) {
            throw new common_1.BadRequestException(`You have to pass fields to update`);
        }
        const updated = await this.spaceJamRepository.updateRecord(record, updateDto);
        const listDto = {};
        const stats = await this.listRecordsByPlayer(listDto, user);
        const toReturn = {};
        toReturn[player1.nickname] = stats[player1.nickname];
        toReturn[player2.nickname] = stats[player2.nickname];
        return toReturn;
    }
    async listRecords(filterDto, user) {
        ...
        const playerObj = class_validator_1.isDefined(player)
            ? await this.playerService.searchPlayer(player)
            : undefined;
        const winnerObj = class_validator_1.isDefined(winner_name)
            ? await this.playerService.searchPlayer(winner_name)
            : undefined;
        const loserObj = class_validator_1.isDefined(loser_name)
            ? await this.playerService.searchPlayer(loser_name)
            : undefined;
        const records = await this.spaceJamRepository.listRecords(filterDto, user, player, loser_name, winner_name);
        records.map((iter) => {
            iter['player1_name'] = iter.player1;
            iter['player2_name'] = iter.player2;
            delete iter.player1;
            delete iter.player2;
            return iter;
        });
    ...
    async listRecordsByPlayer(filterDto, user) {
        ...
        records.forEach((record) => {
            const player1 = record['player1_name'] || record.player1;
            const player2 = record['player2_name'] || record.player2;
            const _2k_rating1 = 0;
            const _2k_rating2 = 0;
...
SpaceJamService = __decorate([
    common_1.Injectable(),
    __param(0, typeorm_1.InjectRepository(space_jam_repository_1.SpaceJamRepository)),
    __metadata("design:paramtypes", [space_jam_repository_1.SpaceJamRepository,
        space_jam_player_service_1.SpaceJamPlayerService])
], SpaceJamService);
exports.SpaceJamService = SpaceJamService;
//# sourceMappingURL=space-jam.service.js.map